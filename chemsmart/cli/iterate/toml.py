"""
CLI subcommand for running iterate jobs from TOML configuration files.
"""
...
def click_toml_iterate_options(f):
    """
    Click options for TOML-based iterate jobs.
    """
...
@click.command(name="toml")
@click_toml_iterate_options
...
def toml(
    ...
    **kwargs,
    ...
    """
    Run iterate jobs from a TOML configuration file.

    This command generates new molecular structures by attaching substituents
    to skeleton molecules at positions specified in the TOML config.

    Examples:

    \b
    chemsmart run iterate toml -f config.toml
    chemsmart run iterate toml -f config.toml -np 4
    chemsmart run iterate toml -f config.toml -o my_output
    chemsmart run iterate toml -g
    chemsmart run iterate toml -g my_config.toml
    """
